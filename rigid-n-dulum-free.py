...
x0 = np.zeros(n * 2 + 2) 
x0[0] = .1
x0[1:n+1] = np.pi + np.linspace(-.05,.1,n)
...
def nonlin_eq(x,t):
    u = np.dot(gains,(ref-x))
    dxdt = np.linalg.solve(M_lambd(*x, u), K_lambd(*x, u))
    return dxdt.T[0]    

def lin_eq(x, t):
    x_der = A_lambd(*x) @ x + B.dot(gains) * (ref-x)
    return x_der

print("Initial derivatives: \n Linear: {} \n Nonlinear: {}".format(lin_eq(x0,1), nonlin_eq(x0,1)))
ax = plt.axes()
...
fig,ax = plt.subplots()
plt.xlim(-lengthsum*1.1, lengthsum*1.1)
